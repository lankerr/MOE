...
import pandas as pd
import h5py
from util.layout import change_layout_np, change_layout_torch
import torch
from torch.nn.functional import avg_pool2d

times=0
# SEVIR Dataset constants
SEVIR_DATA_TYPES = ['vis', 'ir069', 'ir107', 'vil', 'lght'] # 数据格式，一般处理vil
...
# sevir-lr
SEVIR_LR_ROOT_DIR = os.path.join(datacfg.datasets_dir, "sevir_lr")
...
class SEVIRDataLoader:
    ...
    """
    def __init__(self,
                 data_types: Sequence[str] = None, #数据类型
                 seq_len: int = 36, #序列长度，需要小于原始序列长度
                 raw_seq_len: int = 49, #原始序列长度
                 sample_mode: str = 'sequent', #采样模式
                 stride: int = 12, #步长
                 batch_size: int = 1,
                 layout: str = 'NHWT',
                 num_shard: int = 1,
                 ...
                 #datetime_filter=lambda ttt: np.logical_and(ttt.dt.year == 2018, np.logical_and(ttt.dt.month >= 7, ttt.dt.month <= 12)),
                #  catalog_filter = "default",
                 catalog_filter=lambda c: [(s[0] == 'S' and f == 'vil/2017/SEVIR_VIL_STORMEVENTS_2017_0701_1231.h5') or (s[0] == 'S' and f == 'vil/2017/SEVIR_VIL_STORMEVENTS_2017_0101_0630.h5') or (s[0] == 'S' and f == 'vil/2018/SEVIR_VIL_STORMEVENTS_2018_0101_0630.h5') or (s[0] == 'S' and f == 'vil/2018/SEVIR_VIL_STORMEVENTS_2018_0701_1231.h5') or (s[0] == 'S' and f == 'vil/2019/SEVIR_VIL_STORMEVENTS_2019_0101_0630.h5') or (s[0] == 'S' and f == 'SEVIR_VIL_STORMEVENTS_2019_0701_1231.h5') for s, f in zip(c.id, c.file_name)],
                 shuffle: bool = False,
                 shuffle_seed: int = 1,
    ...
    def save_downsampled_dataset(self, save_dir, downsample_dict, verbose=True):
        ...
        import os
        from skimage.measure import block_reduce
        from util.utils import path_splitall
        assert not os.path.exists(save_dir), f"save_dir {save_dir} already exists!"
        os.makedirs(save_dir)
    ...
    def __getitem__(self, index):
        ...
        print(f"Data shape in __getitem__: {data_dict.shape}")
        return data_dict
