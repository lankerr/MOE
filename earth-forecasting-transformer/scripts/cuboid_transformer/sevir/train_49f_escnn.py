"""
Earthformer 49Frame E2CNN 实验训练脚本
======================================
在 Earthformer 49帧 baseline 基础上，将所有 Conv2d 替换为 C4 旋转等变卷积。
Transformer 部分完全不变。

用法:
    cd earth-forecasting-transformer/scripts/cuboid_transformer/sevir
    conda activate rtx3050ti_cu128
    python -u train_49f_escnn.py --cfg cfg_sevir_49frame_escnn.yaml --epochs 10 --save exp_49f_escnn_baseline
"""
...
# === E2CNN 等变卷积替换 ===
from equivariant_layers import patch_model_with_e2cnn, count_e2cnn_layers

_curr_dir = os.path.realpath(os.path.dirname(os.path.realpath(__file__)))
exps_dir = os.path.join(_curr_dir, "experiments")


class CuboidSEVIRE2CNNModule(pl.LightningModule):
    """Earthformer + E2CNN: 将 Conv2d 替换为 C4 旋转等变 R2Conv"""
    ...
    def __init__(self,
        ...
        # 2. 用 e2cnn 替换所有 Conv2d → C4 等变 R2Conv
        patch_model_with_e2cnn(self.torch_nn_module)
        stats = count_e2cnn_layers(self.torch_nn_module)
        print(f"[E2CNN] 层统计: {stats}")
    ...
    def get_logging_config():
        return OmegaConf.create({
            "logging_prefix": "Earthformer_49Frame_E2CNN",
    ...
    def configure_optimizers(self):
        ...
        optimizer_grouped_parameters = [
            {"params": [p for n, p in self.torch_nn_module.named_parameters()
                        if n in decay_parameters], "weight_decay": self.wd},
            {"params": [p for n, p in self.torch_nn_module.named_parameters()
                        if n not in decay_parameters], "weight_decay": 0.0},
        ]
...
# ============================================================
#  Data Module — 与 train_experiment_earthformer.py 相同
# ============================================================
...
def get_parser():
    parser = argparse.ArgumentParser(description='Earthformer 49F E2CNN Baseline')
    parser.add_argument('--save', default='exp_49f_escnn_baseline', type=str)
    parser.add_argument('--gpus', default=1, type=int)
    parser.add_argument('--cfg', default='cfg_sevir_49frame_escnn.yaml', type=str)
...
def main():
    ...
    if args.cfg is not None:
        ...
        dataset_oc = OmegaConf.to_object(CuboidSEVIRE2CNNModule.get_dataset_config())
        micro_batch_size = 1
    ...
    total_num_steps = CuboidSEVIRE2CNNModule.get_total_num_steps(
        epoch=max_epochs, num_samples=dm.num_train_samples, total_batch_size=total_batch_size)

    pl_module = CuboidSEVIRE2CNNModule(
    ...
    if args.test:
        ...
        if os.path.exists(best_path):
            ...
            torch.save(state_dict, os.path.join(pl_module.save_dir, "earthformer_sevir_escnn.pt"))
            print(f"[E2CNN] Best model saved: {pl_module.save_dir}/earthformer_sevir_escnn.pt")
