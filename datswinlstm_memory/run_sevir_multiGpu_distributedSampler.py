...
import os
# [MODIFY] Check PL version for DDP strategy
try:
...
# [FIX] Import path correction: DATSwinLSTM_D_Memory is a file, not a directory
try:
...
# Path to the folder where the pretrained models are saved
CHECKPOINT_PATH = os.environ.get("PATH_CHECKPOINT", os.path.join(os.path.dirname(__file__), "checkpoints"))
...
class SEVIRLightningDataModule(pl.LightningDataModule):
    ...
    def __init__(self, **kwargs):
        ...
        self.start_date = datetime.datetime(*kwargs.get('start_date')) if kwargs.get('start_date') else None
        # 调整日期分割适配 2017 年数据
        self.train_val_split_date = datetime.datetime(*kwargs.get('train_val_split_date', (2017, 4, 1)))
        self.train_test_split_date = datetime.datetime(*kwargs.get('train_test_split_date', (2017, 5, 1)))
        self.end_date = datetime.datetime(*kwargs.get('end_date')) if kwargs.get('end_date') else None
