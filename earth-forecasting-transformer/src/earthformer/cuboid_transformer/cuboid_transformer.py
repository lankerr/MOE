...
class CuboidCrossAttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if use_relative_pos:
            ...
            coords = torch.stack(torch.meshgrid(coords_t, coords_h, coords_w, indexing='ij'))  # 3, maxT, Bh, Bw
    ...
    def forward(self, x, mem, mem_global_vector=None):
        ...
        if self.use_inter_ffn:
            for attn, ffn in zip(self.attn_l, self.ffn_l):
                if self.checkpoint_level >= 2 and self.training:
                    x = x + checkpoint.checkpoint(attn, x, mem, mem_global_vector, use_reentrant=False)
                else:
