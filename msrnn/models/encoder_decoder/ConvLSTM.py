...
class ConvLSTM(ConvRNN):
    ...
    def init_hidden(self, input):
        device = input.device
        hidden_size = (input.size(0), self.cell_param['output_channels'], input.size(-2), input.size(-1))
        h = self.init_paramter(hidden_size, device)
        c = self.init_paramter(hidden_size, device)
        return (h, c)
...
class ConvLSTMCell(ConvRNNCell):
    ...
    def build_model(self):
        ...
        def param(shape):
            return Parameter(torch.empty(*shape, device=device))

        self.w_hi = param((self.output_dim, self.output_dim, *self.state_to_state_kernel_size))
        self.w_hf = param((self.output_dim, self.output_dim, *self.state_to_state_kernel_size))
        self.w_hc = param((self.output_dim, self.output_dim, *self.state_to_state_kernel_size))
        self.w_ho = param((self.output_dim, self.output_dim, *self.state_to_state_kernel_size))

        self.weight_hs = [self.w_hi, self.w_hf, self.w_hc, self.w_ho]

        self.w_xi = param((self.output_dim, self.input_dim, *self.input_to_state_kernel_size))
        self.w_xf = param((self.output_dim, self.input_dim, *self.input_to_state_kernel_size))
        self.w_xo = param((self.output_dim, self.input_dim, *self.input_to_state_kernel_size))
        self.w_xc = param((self.output_dim, self.input_dim, *self.input_to_state_kernel_size))

        self.weights_is = [self.w_xi, self.w_xf, self.w_xc, self.w_xo]

        self.w_ci = param((self.output_dim,))
        self.w_cf = param((self.output_dim,))
        self.w_co = param((self.output_dim,))
        self.weight_cs = [self.w_ci, self.w_cf, self.w_co]

        self.b_i = param((self.output_dim,))
        self.b_f = param((self.output_dim,))
        self.b_o = param((self.output_dim,))
        self.b_c = param((self.output_dim,))
        self.biases = [self.b_i, self.b_f, self.b_c, self.b_o]
