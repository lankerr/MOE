...
class CuboidMovingMNISTPLModule(pl.LightningModule):
    ...
    @staticmethod
    def get_movingmnist_datamodule(micro_batch_size: int = 1, num_workers: int = 8):
        dm = MovingMNISTDataModule(batch_size=micro_batch_size, num_workers=num_workers)
        return dm
    ...
    def on_validation_epoch_end(self):
        # H, W = outputs[0]
        H, W = self.oc.model.target_shape[1], self.oc.model.target_shape[2]
...
def main():
    ...
    dm = CuboidMovingMNISTPLModule.get_movingmnist_datamodule(micro_batch_size=micro_batch_size, num_workers=8)
    dm.prepare_data()
    dm.setup()
