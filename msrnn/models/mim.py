# /data_8t/zhouying/msrnn/models/mim.py
...
class MIM_Cell(nn.Module):
    def __init__(self, input_channel, output_channel, b_h_w, kernel_size, stride, padding):
        ...
        _, self._state_height, self._state_width = b_h_w  # 忽略批次大小
...
class MIM(nn.Module):
    def __init__(self, input_channel, output_channel, b_h_w, kernel_size, stride, padding, embed_layer=None, fc_layer=None):
        super().__init__()
        ...
        # 关键修复：将embed和fc设为类成员，让DDP能追踪
        if embed_layer is not None:
            self.embed = embed_layer
        else:
            # 根据配置创建embed层
            if hasattr(cfg, 'reshape_patch') and cfg.reshape_patch:
                in_channels = cfg.patch_size * cfg.patch_size
            else:
                in_channels = 1
            self.embed = nn.Conv2d(in_channels, input_channel, kernel_size=1, stride=1, padding=0)
        ...
        if fc_layer is not None:
            ...
            # 根据配置创建fc层
            if hasattr(cfg, 'reshape_patch') and cfg.reshape_patch:
                out_channels = cfg.patch_size * cfg.patch_size
            else:
                out_channels = 1
            self.fc = nn.Conv2d(output_channel, out_channels, kernel_size=1, stride=1, padding=0)
        
        print('气象MIM模型初始化 (DDP兼容版)')

    def forward(self, x, m, layer_hiddens):
        # 关键修复：使用成员变量而不是参数
        x = self.embed(x)
        next_layer_hiddens = []
        
        # 关键修复：简化decouple_loss，避免维度错误
        decouple_loss = torch.tensor(0.0, device=x.device)
