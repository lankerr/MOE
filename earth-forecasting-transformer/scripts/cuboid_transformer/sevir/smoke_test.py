...
Usage:
    ...
    python smoke_test.py --model gmr_patch --test_run
"""
...
try:
    ...
    print("  ✓ Physics attention modules imported")
...
try:
    ...
    for path in gmr_paths:
        try:
            ...
            print(f"  ⚠ {path} not found (may need to add to path)")
except Exception as e:
    print(f"  ✗ GMR check failed: {e}")
...
def test_pgsa_forward():
    ...
    print(f"  ✓ PGSA forward: {elapsed*1000:.2f}ms")
...
def test_dpcba_forward():
    ...
    assert out.shape == x.shape, f"Shape mismatch: {out.shape} != {x.shape}"
    print(f"  ✓ DPCBA forward: {elapsed*1000:.2f}ms")
...
print(f"  ✓ WSD scheduler implemented")
...
# Test 5: Smoke test with dummy data
print()
    ...
    class DummyModel(nn.Module):
        def __init__(self):
            ...
            self.encoder = nn.Sequential(
                nn.Conv2d(1, 32, 4, stride=4),  # 384 -> 96
                nn.Conv2d(32, 64, 3, stride=3),  # 96 -> 32
            )
            self.decoder = nn.Sequential(
                nn.ConvTranspose2d(64, 32, 3, stride=3),  # 32 -> 96
                nn.ConvTranspose2d(32, 1, 4, stride=4),   # 96 -> 384
            )
