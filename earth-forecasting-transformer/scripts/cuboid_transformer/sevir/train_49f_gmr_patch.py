"""
Earthformer 49Frame GMR-Conv Equi-ViT Patch Embedding 实验
==========================================================
与 GMR baseline (train_49f_gmr.py) 不同:
  - GMR baseline: 保持原始 3级 stack_conv 结构，只换算子 Conv2d→GMR_Conv2d
  - 本实验: 用 ViT 风格的 2级大步长 GMR-Conv 重新设计 patch embedding

编码器: GMR(1→16, stride4) + GMR(16→64, stride3) = 384→96→32  (2层, 12× 下采样)
解码器: Up×2+GMR + Up×2+GMR + Up×3+GMR = 32→64→128→384          (3层, 12× 上采样)
Transformer: 完全不变

用法:
    cd earth-forecasting-transformer/scripts/cuboid_transformer/sevir
    conda activate rtx3050ti_cu128
    python -u train_49f_gmr_patch.py --cfg cfg_sevir_49frame_gmr_patch.yaml --epochs 10 --save exp_49f_gmr_patch_baseline
"""
...
class CuboidSEVIRGMRPatchModule(pl.LightningModule):
    """Earthformer + GMR Equi-ViT Patch Embedding"""
    ...
    def __init__(self, total_num_steps: int, oc_file: str = None, save_dir: str = None):
        ...
        # 1. 先创建标准 Earthformer 模型 (stack_conv)
        ...
        # 2. 替换 initial_encoder + final_decoder → GMR patch embed
        patch_model_with_gmr_embed(self.torch_nn_module,
                                   in_chans=model_cfg["input_shape"][-1],
                                   embed_dim=model_cfg["base_units"],
                                   mid_dim=16, out_dim=4)
        stats = count_gmr_embed_layers(self.torch_nn_module)
        print(f"[GMR-PatchEmbed] 层统计: {stats}")
    ...
    def get_logging_config():
        return OmegaConf.create({
            "logging_prefix": "Earthformer_49Frame_GMR_PatchEmbed",
    ...
    def configure_optimizers(self):
        ...
        optimizer_grouped_parameters = [
            ...
            {"params": sigma_params, "weight_decay": 0.0, "lr": self.lr * 0.1},
        ]
...
def get_parser():
    parser = argparse.ArgumentParser(description='Earthformer 49F GMR Patch Embed')
    parser.add_argument('--save', default='exp_49f_gmr_patch_baseline', type=str)
    parser.add_argument('--gpus', default=1, type=int)
    parser.add_argument('--cfg', default='cfg_sevir_49frame_gmr_patch.yaml', type=str)
...
def main():
    ...
    if args.cfg is not None:
        ...
        dataset_oc = OmegaConf.to_object(CuboidSEVIRGMRPatchModule.get_dataset_config())
        micro_batch_size = 1
    ...
    total_num_steps = CuboidSEVIRGMRPatchModule.get_total_num_steps(
        epoch=max_epochs, num_samples=dm.num_train_samples, total_batch_size=total_batch_size)

    pl_module = CuboidSEVIRGMRPatchModule(
    ...
    if args.test:
        ...
        if os.path.exists(best_path):
            ...
            torch.save(state_dict, os.path.join(pl_module.save_dir, "earthformer_sevir_gmr_patch.pt"))
            print(f"[GMR-PatchEmbed] Best model saved: {pl_module.save_dir}/earthformer_sevir_gmr_patch.pt")
