...
# Windows: matplotlib 默认用 TkAgg 后端，在 subprocess 中会触发
# "Tcl_AsyncDelete: async handler deleted by the wrong thread" 致命错误。
# 必须在任何 matplotlib 导入之前切到非交互后端。
import matplotlib
matplotlib.use('Agg')
# Suppress 'triton not found' logging message from torch.utils.flop_counter
# (triton is not available on Windows, this is expected and harmless)
import logging
...
# PyTorch 2.11+ nightly: allowlist OmegaConf types for checkpoint loading
# (PL checkpoints contain DictConfig/ListConfig/Container objects)
import omegaconf
# PyTorch 2.6+ sets weights_only=True by default which breaks OmegaConf loading.
# Since we trust our local checkpoints, we monkeypatch torch.load to force weights_only=False
import torch
...
class CuboidSEVIRPLModule(pl.LightningModule):
    ...
    def get_dataset_config():
        ...
        oc.layout = "NTHWC"
        oc.start_date = (2017, 6, 13)
        oc.train_val_split_date = (2017, 8, 1)
        oc.train_test_split_date = (2017, 8, 8)
        oc.end_date = (2017, 8, 16)
        oc.metrics_mode = "0"
    ...
    def get_logging_config():
        ...
        oc.use_wandb = False  # 禁用 wandb (RTX 5070 本地测试)
        return oc
    ...
    def set_trainer_kwargs(self, **kwargs):
        ...
        # Use TQDMProgressBar with proper refresh rate for clean terminal output
        ...
        ret = dict(
            ...
            # track_grad_norm=self.oc.logging.track_grad_norm, # Deprecated in PL 2.0
            # save
        ...
        if kwargs.get('devices', 1) > 1:
            ret['strategy'] = "ddp"
        # Disable default model summary to avoid triton flop counter warning
        # and precision 16-mixed model summary warning
        ret['enable_model_summary'] = False
    ...
    def training_step(self, batch, batch_idx):
        ...
        x = data_seq[tuple(self.in_slice)].contiguous()
        y = data_seq[tuple(self.out_slice)].contiguous()
        y_hat, loss = self(x, y)
        # 收集 MoE 辅助损失 (balance_loss + ortho_loss)
        # 直接加 aux_loss (当 weight=0 时 aux_loss=0.0, 加 0 不影响梯度)
        # 避免 .item() 调用导致的 GPU→CPU 同步开销
...
def get_parser():
    ...
    parser.add_argument('--exp', default='baseline', type=str, help='Experiment name for ablation')
    parser.add_argument('--epochs', default=None, type=int, help='Override number of training epochs')
    return parser
...
class AlignedSEVIRDataModule(LightningDataModule):
    ...
    def collate_fn(self, batch):
        ...
        return {"vil": batch_tensor}

    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.micro_batch_size, shuffle=True, drop_last=True, collate_fn=self.collate_fn, num_workers=self.num_workers)

    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=self.micro_batch_size, shuffle=False, drop_last=False, collate_fn=self.collate_fn, num_workers=self.num_workers)
...
def main():
    ...
    if args.cfg is not None:
        ...
        max_epochs = args.epochs if getattr(args, 'epochs', None) is not None else oc_from_file.optim.max_epochs
    ...
    # Windows: 多进程 worker 会额外加载 PyTorch+CUDA DLL，在 16GB 内存机器上极易 OOM。
    # 跑通的原版 (train_cuboid_sevir.py) 用的就是 num_workers=0。
    num_workers = 0
    ...
    if getattr(args, 'exp', 'baseline') != 'baseline':
        from experiment_factory_earthformer import apply_experiment
        apply_experiment(pl_module.torch_nn_module, args.exp)
        
    trainer_kwargs = pl_module.set_trainer_kwargs(
        devices=args.gpus,
        accumulate_grad_batches=accumulate_grad_batches,
        max_epochs=max_epochs,  # 用 --epochs 参数覆盖 YAML 的 max_epochs
    )
    # print("DEBUG: trainer_kwargs =", trainer_kwargs)
    ...
    if args.pretrained:
        ...
        # 检查 checkpoint 的 epoch 是否已达到 max_epochs
        # 如果是，跳过 fit，直接用已有的 best checkpoint 做 test
        _skip_fit = False
        if ckpt_path is not None:
            _ckpt_data = torch.load(ckpt_path, map_location="cpu")
            _ckpt_epoch = _ckpt_data.get("epoch", 0)
            if _ckpt_epoch >= max_epochs:
                print(f"  [跳过训练] checkpoint 已在 epoch {_ckpt_epoch}, 目标 max_epochs={max_epochs}")
                _skip_fit = True
            del _ckpt_data
        ...
        if os.path.exists(best_path):
            ...
            print("  [警告] 未找到任何 checkpoint，跳过 test")
